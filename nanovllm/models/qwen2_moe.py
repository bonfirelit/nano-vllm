...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(
        ...
        self.experts = nn.ModuleList(
            [Qwen2MoeMLP(self.hidden_size, self.intermediate_size, self.hidden_act) for _ in range(self.num_experts)]
        )

        self.shared_expert = Qwen2MoeMLP(self.hidden_size, self.shared_expert_intermediate_size, self.hidden_act)
    ...
    def forward(
        ...
        shared_expert_weight = torch.sigmoid(self.shared_expert_gate(hidden_states))
        final_hidden_states += shared_expert_output * shared_expert_weight
