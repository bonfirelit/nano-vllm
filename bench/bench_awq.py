#!/usr/bin/env python3
"""
Benchmark script for AWQ quantized models.
...
# Configuration
# ============================================
MODEL_PATH = os.path.expanduser("~/huggingface/Qwen3-0.6B-AWQ/")
...
def benchmark_awq(model_path: str) -> dict:
    """
    Benchmark AWQ quantized model.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Testing: AWQ Quantized Model")
...
def print_summary(results: dict):
    """Print summary table."""
    print(f"\n{'='*60}")
    print(f"{'AWQ MODEL BENCHMARK RESULTS':^60}")
    print(f"{'='*60}")
...
def main():
    ...
    if not os.path.isdir(MODEL_PATH):
        print(f"Error: AWQ model not found at {MODEL_PATH}")
    ...
    results = benchmark_awq(MODEL_PATH)
